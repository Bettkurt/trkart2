...
CREATE TABLE "Wallets" (
    ...
    "WalletNumber" CHAR(10) NOT NULL UNIQUE,
    "CustomerID" INT NOT NULL,
    "Balance" DECIMAL(18, 2) NOT NULL DEFAULT 0.00,
    "Status" INT NOT NULL,
    "CreatedAt" TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    ...
    CONSTRAINT "FK_Wallets_Customers_CustomerID"
        FOREIGN KEY ("CustomerID")
        REFERENCES "Customers" ("CustomerID")
        ON DELETE CASCADE,
    
    CONSTRAINT "CHK_Wallet_Balance_NotNegative"
        CHECK ("Balance" >= 0)
);

-- Create index for better query performance
CREATE UNIQUE INDEX "IX_Wallets_CustomerID" 
ON "Wallets" ("CustomerID");
...
CREATE TRIGGER TR_Wallets_UpdateTimestamp
...
CREATE TABLE "Transaction" (
    ...
    -- 0: Load, 1: TopUp, 2: Refund, 3: TransferIn, 4: TransferOut, 5: Pay, 6: SystemTransferIn, 7: SystemTransferOut, 8: WalletLoad, 9: WalletPay, 10: CardToWalletTransfer, 11: WalletToCardTransfer
    "TransactionType" INT NOT NULL CHECK ("TransactionType" BETWEEN 0 AND 11),
    ...
    CONSTRAINT "FK_Transaction_Wallets_WalletID"
        FOREIGN KEY ("WalletID")
        REFERENCES "Wallets" ("WalletID")
        ON DELETE CASCADE,
    
    CONSTRAINT "CHK_Transaction_CardOrWallet"
        CHECK (("WalletID" IS NULL AND "CardID" IS NOT NULL) OR 
               ("WalletID" IS NOT NULL AND "CardID" IS NULL) OR
               ("WalletID" IS NOT NULL AND "CardID" IS NOT NULL AND "TransactionType" IN (10, 11))), -- Allow both for transfer types
...
-- Create the transaction processing function with separated wallet and card logic
...
DECLARE
    ...
    v_card_balance DECIMAL(18, 2);
    v_is_wallet_transaction BOOLEAN;
    v_is_card_transaction BOOLEAN;
    v_is_transfer_between_card_wallet BOOLEAN;
BEGIN
    -- Determine the type of transaction
    v_is_wallet_transaction := (NEW."WalletID" IS NOT NULL);
    v_is_card_transaction := (NEW."CardID" IS NOT NULL);
    v_is_transfer_between_card_wallet := (v_is_wallet_transaction AND v_is_card_transaction);
    
    -- Validate transaction type based on the involved entities
    IF v_is_transfer_between_card_wallet THEN
        -- For transfers between card and wallet, both must be provided and type must be 10 or 11
        IF NOT (NEW."TransactionType" IN (10, 11)) THEN
            NEW."TransactionStatus" := 'Denied';
            RETURN NEW;
        END IF;
    ELSIF v_is_wallet_transaction AND NOT (NEW."TransactionType" BETWEEN 8 AND 11) THEN
        -- Wallet transactions must be of type 8-11
        NEW."TransactionStatus" := 'Denied';
        RETURN NEW;
    ELSIF v_is_card_transaction AND NOT (NEW."TransactionType" BETWEEN 0 AND 7) THEN
        -- Card transactions must be of type 0-7
        NEW."TransactionStatus" := 'Denied';
        RETURN NEW;
    END IF;
    ...
    -- Handle wallet transactions (types 8-11)
    IF v_is_wallet_transaction THEN
        ...
        -- Handle card-to-wallet transfer (type 10)
        IF NEW."TransactionType" = 10 AND v_is_card_transaction THEN
            -- Get card balance with lock
            SELECT "Balance", "CardStatus" 
            INTO v_card_balance, v_current_card_status
            FROM "UserCard"
            WHERE "CardID" = NEW."CardID"
            FOR UPDATE;
            
            -- Check if card is active and has sufficient balance
            IF v_current_card_status = 4 AND v_card_balance >= NEW."Amount" THEN
                -- Deduct from card
                UPDATE "UserCard"
        ...
        -- Process other wallet transaction types
        CASE NEW."TransactionType"
            WHEN 8 THEN -- WalletLoad
                -- Add to wallet
                UPDATE "Wallets"
                SET "Balance" = "Balance" + NEW."Amount"
                WHERE "WalletID" = NEW."WalletID";
            ...
            WHEN 11 THEN -- WalletToCardTransfer
                -- Check if wallet has sufficient balance
                IF v_wallet_balance >= NEW."Amount" THEN
                    -- Deduct from wallet
                    UPDATE "Wallets"
    ...
    -- Handle card transactions (types 0-7)
    IF v_is_card_transaction THEN
        -- Get current card balance and status with lock
        SELECT "Balance", "CardStatus" 
        ...
        -- Handle system transfers (bypass some checks for system operations)
        IF NEW."TransactionType" = 6 THEN -- SystemTransferIn
            -- Add to card
            UPDATE "UserCard"
            SET "Balance" = "Balance" + NEW."Amount"
            WHERE "CardID" = NEW."CardID";
            ...
            NEW."TransactionStatus" := 'Approved';
            RETURN NEW;
        ELSIF NEW."TransactionType" = 7 THEN -- SystemTransferOut
        ...
        -- For non-system transactions, check card status
        IF v_current_card_status < 3 THEN -- 0: Deactivated, 1: Expired, 2: Lost
            NEW."TransactionStatus" := 'Denied';
            RETURN NEW;
        END IF;
        ...
        CASE NEW."TransactionType"
            WHEN 0 THEN -- Load
                -- For load transactions, just add the amount
                UPDATE "UserCard"
                SET "Balance" = "Balance" + NEW."Amount"
                WHERE "CardID" = NEW."CardID";
                
                -- Activate inactive cards on first load
                IF v_current_card_status = 3 THEN -- 3: Inactive
                    UPDATE "UserCard"
                    SET "CardStatus" = 4 -- 4: Active
                    WHERE "CardID" = NEW."CardID";
                END IF;
                
                NEW."TransactionStatus" := 'Approved';
                
            WHEN 1 THEN -- TopUp (with fee)
                -- Validate fee amount
                IF NEW."FeeAmount" IS NULL OR NEW."FeeAmount" < 0 OR NEW."FeeAmount" > NEW."Amount" THEN
                    NEW."TransactionStatus" := 'Denied';
                    RETURN NEW;
                END IF;
                
                -- Calculate net amount (amount - fee)
                v_net_amount := NEW."Amount" - NEW."FeeAmount";
                
                -- Add net amount to card
                UPDATE "UserCard"
            ...
            WHEN 4 THEN -- TransferOut
                -- Check if card has sufficient balance and is active
                IF v_current_card_status = 4 AND v_current_balance >= NEW."Amount" THEN
                    -- Deduct from card
                    UPDATE "UserCard"
                    SET "Balance" = "Balance" - NEW."Amount"
                    WHERE "CardID" = NEW."CardID";
            ...
            ELSE
                -- Unknown transaction type for cards
                NEW."TransactionStatus" := 'Denied';
        END CASE;
    END IF;
...
-- Add comments for transaction types
COMMENT ON COLUMN "Transaction"."TransactionType" IS 
'0: Load, 1: TopUp, 2: Refund, 3: TransferIn, 4: TransferOut, 5: Pay, 6: SystemTransferIn, 7: SystemTransferOut, 8: WalletLoad, 9: WalletPay, 10: CardToWalletTransfer, 11: WalletToCardTransfer';

-- Drop and recreate indexes for transaction table to include wallet-related indexes
DROP INDEX IF EXISTS "IDX_Transaction_CardID";
DROP INDEX IF EXISTS "IDX_Transaction_TransferTransactionID";
DROP INDEX IF EXISTS "IDX_Transaction_Amount";
DROP INDEX IF EXISTS "IDX_Transaction_TransactionDate";

-- Recreate indexes with better organization
CREATE INDEX "IDX_Transaction_CardID" ON "Transaction"("CardID") WHERE "CardID" IS NOT NULL;
CREATE INDEX "IDX_Transaction_WalletID" ON "Transaction"("WalletID") WHERE "WalletID" IS NOT NULL;
CREATE INDEX "IDX_Transaction_TransferTransactionID" ON "Transaction"("TransferTransactionID") WHERE "TransferTransactionID" IS NOT NULL;
CREATE INDEX "IDX_Transaction_TransactionDate" ON "Transaction"("TransactionDate");
CREATE INDEX "IDX_Transaction_TransactionType" ON "Transaction"("TransactionType");
CREATE INDEX "IDX_Transaction_TransactionStatus" ON "Transaction"("TransactionStatus");
